...
<?php
    ...
    echo '<p class="header">Edit Individual Meal / <a href="addMeal.php?brand='.$meal['brand'].'&category='.$meal['category'].'"> Add '.$pageName.' </a> / <a href="meals.php?brand='.$meal['brand'].'&category='.$meal['category'].'"> '.$pageName.'s </a></P>';
            ...
            if(isset($_POST['mealName']) && ($_POST['mealName'] != "")){
                ...
                if((isset($_POST['rprice']) && ($_POST['rprice'] != "") && is_numeric($_POST['rprice'])) || (isset($_POST['mprice']) && ($_POST['mprice'] != "") && is_numeric($_POST['mprice'])) || (isset($_POST['lprice']) && ($_POST['lprice'] != "") && $_POST['lprice'])){
                    ...
                    if($check){
                        ...
                        if($ingredient3 != ""){
                            if(isset($_POST['ingredient3ID']) && is_numeric($_POST['ingredient3ID'])){
                                $ingredient = new Ingredient($ingredient3, $mealID);
                                $ingredient->update($_POST['ingredient3ID']);
                            }else{
                                $ingredient = new Ingredient($ingredient3, $mealID);
                                $ingredient->add();
                            }
                        ...
                        echo '<div class="greenMessage">
                            ...
                            header('Refresh: 10; mealDetails.php?mealID='.$meal['mealID'].'&brand='.$meal['brand'].'&category='.$meal['category'].'');
                    }else{
                        echo '<div class="redMessage">
                                <p>The Meal is not Updated Please try Again !</p>
                            </div>';
                            header('Refresh: 10; editMeal.php?mealID='.$meal['mealID'].'&brand='.$meal['brand'].'&category='.$meal['category'].'');
                    }
                }else{
                    echo '<div class="redMessage">
    ...
    <div class="form-container">
        <form action="<?php echo $_SERVER['PHP_SELF']?>" method="POST" enctype="multipart/form-data" accept-charset="utf-8">
            <?php
                ...
                for($i = 0 ; $i < (3 - count($ingredients)) ; $i++){
                    echo '<div class="form-group col-md-12">
                            <label >Ingredeient</label>
